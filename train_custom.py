...
def train_model(model_size="s", epochs=100, batch_size=16, img_size=640, device="0"):
    ...
    # Đếm ảnh
    train_count, _val_count, _test_count = count_images()
    ...
    cmd = [
        ...
        device,
        "--name",
        f"traffic_signs_{model_size}",
        "--project",
        "runs/train",
    ]

    print("🚀 Bắt đầu training với:")
...
def main():
    ...
    parser.add_argument(
        "--action", choices=["setup", "train", "test", "all"], default="all", help="Hành động thực hiện"
    )
    parser.add_argument(
        "--model",
        choices=["n", "s", "m", "l", "x"],
