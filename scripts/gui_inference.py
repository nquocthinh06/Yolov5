...
class TrafficPeopleDetectionGUI:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            header_frame,
            text="🚗 YOLOv5x - Phát hiện Phương tiện & Con người (Độ chính xác cao nhất)",
            font=("Arial", 16, "bold"),
            fg="white",
            bg="#2c3e50",
        )
        ...
        left_frame = tk.LabelFrame(
            images_frame, text="📁 Ảnh gốc", font=("Arial", 12, "bold"), bg="white", fg="#2c3e50", relief="raised", bd=2
        )
        ...
        results_frame = tk.LabelFrame(
            bottom_frame,
            text="📊 Kết quả phân tích chi tiết",
            font=("Arial", 16, "bold"),
            bg="white",
            fg="#2c3e50",
            relief="raised",
    ...
    def create_image_panel(self, parent, panel_type):
        ...
        if panel_type == "input":
            self.input_image_label = tk.Label(
                image_frame, text="Chưa có ảnh", font=("Arial", 14), bg="#ecf0f1", fg="#7f8c8d"
            )
            self.input_image_label.pack(expand=True)

            # File info
            self.file_info_label = tk.Label(parent, text="", font=("Arial", 10), bg="white", fg="#7f8c8d")
            self.file_info_label.pack(pady=5)
        else:
            self.result_image_label = tk.Label(
                image_frame, text="Chờ xử lý...", font=("Arial", 14), bg="#ecf0f1", fg="#7f8c8d"
            )
    ...
    def create_advanced_controls(self, parent):
        ...
        self.status_label = tk.Label(
            parent,
            text="⏳ Đang tải YOLOv5x (Model chính xác nhất)...",
            font=("Arial", 12, "bold"),
            fg="white",
            bg="#34495e",
        )
        ...
        tk.Label(settings_frame, text="Độ tin cậy:", font=("Arial", 11), fg="white", bg="#34495e").grid(
        ...
        # IoU threshold
        tk.Label(settings_frame, text="IoU:", font=("Arial", 11), fg="white", bg="#34495e").grid(
        ...
        tk.Label(model_frame, text="Model:", font=("Arial", 11), fg="white", bg="#34495e").pack()
        ...
        self.process_btn = tk.Button(
            parent,
            text="🚀 Phát hiện (Độ chính xác cao)",
            font=("Arial", 12, "bold"),
            bg="#e74c3c",
            fg="white",
    ...
    def create_traffic_results_panel(self, parent):
        ...
        tk.Label(left_col, text="📋 Danh sách phát hiện:", font=("Arial", 14, "bold"), bg="white", fg="#2c3e50").pack(
            anchor="w"
        )
        ...
        self.results_text = tk.Text(
            results_text_frame, height=20, font=("Arial", 12), bg="white", relief="flat", wrap=tk.WORD
        )
        ...
        tk.Label(right_col, text="📊 Thống kê tổng quan:", font=("Arial", 14, "bold"), bg="white", fg="#2c3e50").pack(
            anchor="w"
        )

        self.stats_text = tk.Text(right_col, height=20, font=("Arial", 12), bg="white", relief="flat", wrap=tk.WORD)
        ...
        self.vehicle_text = tk.Text(vehicle_tab, height=25, font=("Arial", 12), bg="white", relief="flat", wrap=tk.WORD)
        ...
        self.people_text = tk.Text(people_tab, height=25, font=("Arial", 12), bg="white", relief="flat", wrap=tk.WORD)
        ...
        def change_model():
            try:
                ...
                self.status_label.config(
                    text=f"✅ {model_name} sẵn sàng! ({accuracy_info.get(model_name, 'Không rõ')})"
                )
                self.process_btn.config(state="normal")

            except Exception as e:
                self.status_label.config(text=f"❌ Lỗi: {e!s}")
                messagebox.showerror("Lỗi", f"Không thể tải model {model_name}: {e!s}")
    ...
    def select_video(self):
        """Chọn video."""
        file_path = filedialog.askopenfilename(
            title="Chọn video", filetypes=[("Video", "*.mp4 *.avi *.mov *.mkv *.wmv"), ("Tất cả", "*.*")]
        )
    ...
    def update_detection_results(self, people, vehicles, traffic_signs):
        ...
        self.results_text.insert(tk.END, "🎯 TỔNG QUAN PHÁT HIỆN:\n")
    ...
    def update_statistics(self, people, vehicles, traffic_signs):
        ...
        if people:
            ...
            self.stats_text.insert(tk.END, "👥 THỐNG KÊ CON NGƯỜI:\n")
            self.stats_text.insert(tk.END, f"   • Số lượng: {len(people)}\n")
            self.stats_text.insert(
                tk.END, f"   • Độ tin cậy TB: {sum(people_confidences) / len(people_confidences):.1f}%\n"
            )
        ...
        if vehicles:
            ...
            self.stats_text.insert(tk.END, "🚗 THỐNG KÊ PHƯƠNG TIỆN:\n")
            self.stats_text.insert(tk.END, f"   • Tổng số: {len(vehicles)}\n")
            self.stats_text.insert(tk.END, "   • Loại phương tiện:\n")
    ...
    def process_video(self, video_path):
        ...
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể xử lý video: {e!s}")
    ...
    def create_video_processing_window(self, video_path):
        ...
        title_label = tk.Label(
            title_frame, text=f"🎬 {os.path.basename(video_path)}", font=("Arial", 16, "bold"), fg="white", bg="#2c3e50"
        )
        title_label.pack(side="left")

        # Thông tin video ngay trên header
        self.video_header_info = tk.Label(
            title_frame, text="Chuẩn bị...", font=("Arial", 10), fg="#ecf0f1", bg="#2c3e50"
        )
        ...
        self.video_label = tk.Label(
            video_frame, text="Đang tải video...", font=("Arial", 14), bg="#ecf0f1", fg="#7f8c8d"
        )
        ...
        tk.Label(stats_panel, text="📊 Thống kê Real-time", font=("Arial", 11, "bold"), fg="white", bg="#34495e").pack(
            pady=10
        )

        self.realtime_fps_label = tk.Label(stats_panel, text="FPS: --", font=("Arial", 10), fg="#ecf0f1", bg="#34495e")
        self.realtime_fps_label.pack(pady=5)

        self.realtime_detections_label = tk.Label(
            stats_panel, text="Phát hiện: 0", font=("Arial", 10), fg="#ecf0f1", bg="#34495e"
        )
        self.realtime_detections_label.pack(pady=5)

        self.realtime_time_label = tk.Label(
            stats_panel, text="Thời gian: 00:00", font=("Arial", 10), fg="#ecf0f1", bg="#34495e"
        )
        ...
        self.current_time_label = tk.Label(time_frame, text="00:00", font=("Arial", 9), fg="white", bg="#34495e")
        self.current_time_label.pack(side="left")

        self.total_time_label = tk.Label(time_frame, text="/ 00:00", font=("Arial", 9), fg="white", bg="#34495e")
        ...
        self.video_progress = ttk.Progressbar(progress_frame, mode="determinate", length=800, style="TProgressbar")
        ...
        tk.Label(skip_frame, text="Frame skip:", font=("Arial", 9), fg="white", bg="#34495e").pack(
            side="left", padx=(0, 5)
        )

        self.frame_skip_var = tk.IntVar(value=5)
        skip_spin = tk.Spinbox(
            skip_frame,
            from_=1,
            to=30,
            width=5,
        ...
        self.video_info_label = tk.Label(
            btn_frame, text="Chuẩn bị xử lý video...", font=("Arial", 10), fg="white", bg="#34495e"
        )
        ...
        self.video_results_text = tk.Text(
            frames_tab, height=15, font=("Arial", 11), bg="white", relief="flat", wrap=tk.WORD
        )
        ...
        self.video_summary_text = tk.Text(
            summary_tab, height=15, font=("Arial", 11), bg="white", relief="flat", wrap=tk.WORD
        )
        ...
        self.frame_times = []
        self.video_stats = {"total_detections": 0, "people_count": 0, "vehicle_count": 0, "frames_processed": 0}
        ...
        action_frame = tk.LabelFrame(
            actions_tab,
            text="📦 CÁC CHỨC NĂNG XUẤT KẾT QUẢ",
            font=("Arial", 12, "bold"),
            bg="#e8f4fd",
            fg="#2c3e50",
            relief="raised",
        ...
        # Hàng 1: Báo cáo
        tk.Label(action_row1, text="📊 BÁO CÁO:", font=("Arial", 11, "bold"), bg="#e8f4fd", fg="#2c3e50").pack(
            side="left", padx=(0, 15)
        )

        self.instant_report_btn = tk.Button(
            action_row1,
            text="📋 Tạo báo cáo ngay",
            font=("Arial", 11, "bold"),
            bg="#9b59b6",
            fg="white",
            command=self.create_instant_report,
            relief="raised",
            padx=20,
            pady=8,
            bd=2,
        ...
        tk.Label(action_row2, text="🎬 VIDEO:", font=("Arial", 11, "bold"), bg="#e8f4fd", fg="#2c3e50").pack(
            side="left", padx=(0, 15)
        )

        self.export_video_btn = tk.Button(
            action_row2,
            text="🎬 XUẤT VIDEO ĐÃ XỬ LÝ",
            font=("Arial", 12, "bold"),
            bg="#e74c3c",
            fg="white",
        ...
        note_label = tk.Label(
            action_frame,
            text="💡 Lưu ý: Nút 'XUẤT VIDEO ĐÃ XỬ LÝ' tạo file MP4 với khoanh vùng màu sắc",
...
"""
    ...
    def process_video_thread(self, video_path):
        ...
        try:
            ...
            self.video_window.after(
                0, lambda: self.video_info_label.config(text=f"Đang xử lý: {width}x{height} @ {fps:.1f}fps")
            )
            ...
            while self.video_processing:
                ...
                if frame_count % frame_skip == 0:
                    ...
                    self.video_window.after(
                        0,
                        lambda fps_val=self.processing_fps, det=num_detections: self.update_realtime_stats(
                            fps_val, det, frame_count
                        ),
                    )
                ...
                self.video_window.after(
                    0, lambda p=progress_percent, f=frame_count, t=current_time_pos: self.update_video_progress(p, f, t)
                )
            ...
            self.video_window.after(0, lambda: messagebox.showerror("Lỗi", f"Lỗi xử lý video: {e!s}"))
            self.video_window.after(0, self.video_processing_finished)
    ...
    def process_video_frame_results(self, results, frame_number, frame):
        ...
        try:
            ...
            if result.boxes is not None and len(result.boxes) > 0:
                ...
                frame_info = (
                    f"Frame {frame_number}: {len(result.boxes)} đối tượng "
    ...
    def update_video_progress(self, progress_percent, frame_number, current_time_pos=0):
        ...
        self.video_info_label.config(
            text=f"Frame {frame_number}/{self.total_frames} ({progress_percent:.1f}%) | FPS: {self.processing_fps:.1f}"
            if self.processing_fps > 0
            else f"Frame {frame_number}/{self.total_frames} ({progress_percent:.1f}%)"
        )
...
"""
    ...
    def save_video_results(self):
        ...
        try:
            save_path = filedialog.asksaveasfilename(
                ...
                filetypes=[("Text files", "*.txt"), ("CSV files", "*.csv"), ("Tất cả", "*.*")],
            )
            ...
            messagebox.showerror("Lỗi", f"Không thể lưu kết quả: {e!s}")
    ...
    def create_instant_report(self):
        ...
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể tạo báo cáo: {e!s}")
    ...
    def open_results_folder(self):
        ...
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể mở thư mục: {e!s}")
    ...
    def export_processed_video(self, input_video_path):
        ...
        try:
            ...
            output_path = filedialog.asksaveasfilename(
                ...
                filetypes=[("MP4 Video", "*.mp4"), ("AVI Video", "*.avi"), ("Tất cả", "*.*")],
            )
            ...
            progress_window.geometry(
                "+%d+%d" % (self.video_window.winfo_rootx() + 350, self.video_window.winfo_rooty() + 150)
            )

            tk.Label(
                progress_window,
                text="🎬 Đang xuất video với khoanh vùng nâng cao...",
                font=("Arial", 12, "bold"),
                bg="#f0f0f0",
            ).pack(pady=15)
            ...
            export_info_label = tk.Label(progress_window, text="Chuẩn bị...", font=("Arial", 10), bg="#f0f0f0")
            export_info_label.pack(pady=5)

            # Thêm thông tin chi tiết
            detail_label = tk.Label(progress_window, text="", font=("Arial", 9), bg="#f0f0f0", fg="#666")
            ...
            def export_thread():
                try:
                    ...
                    video_stats = {"total_people": 0, "total_vehicles": 0, "frames_with_detections": 0}
                    ...
                    while True:
                        ...
                        if frame_count % 5 == 0:  # Cập nhật mỗi 5 frame
                            ...
                            progress_window.after(
                                0,
                                lambda p=progress_percent, f=frame_count, d=total_detections: (
                    ...
                    progress_window.after(
                        0, lambda: (progress_window.destroy(), messagebox.showerror("Lỗi", f"Lỗi xuất video: {e!s}"))
                    )
            ...
            messagebox.showerror("Lỗi", f"Không thể xuất video: {e!s}")
    ...
    def create_enhanced_annotation(self, frame, result):
        ...
        for i, box in enumerate(result.boxes):
            ...
            # Vẽ text
            cv2.putText(annotated_frame, label, (x1 + 5, y1 - 5), font, font_scale, (0, 0, 0), font_thickness)

            # Thêm ID số thứ tự
            cv2.putText(annotated_frame, f"#{i + 1}", (x1, y2 + 20), font, 0.5, color, 2)
        ...
        cv2.putText(annotated_frame, info_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    ...
    def show_export_success(self, video_path, report_path, stats, total_detections):
        ...
        success_window.geometry(
            "+%d+%d" % (self.video_window.winfo_rootx() + 350, self.video_window.winfo_rooty() + 100)
        )
        ...
        tk.Label(
            header_frame, text="✅ Xuất video thành công!", font=("Arial", 16, "bold"), fg="white", bg="#27ae60"
        ).pack(expand=True)
...
• Thông tin timestamp và tổng quan"""
        ...
        info_label = tk.Label(
            content_frame, text=info_text, font=("Arial", 11), bg="#f0f0f0", justify="left", anchor="w"
        )
    ...
    def save_result(self):
        ...
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể lưu ảnh: {e!s}")
    ...
    def export_data(self):
        ...
        try:
            ...
            messagebox.showerror("Lỗi", f"Không thể xuất dữ liệu: {e!s}")


def main():
    root = tk.Tk()
    TrafficPeopleDetectionGUI(root)
    root.mainloop()
