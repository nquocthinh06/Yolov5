...
class SimpleYOLOGUI:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(header_frame, text="🚀 YOLOv5 - Phát hiện đối tượng", 
        ...
        # Left panel - Input
        left_frame = tk.LabelFrame(main_frame, text="📁 Ảnh gốc", font=('Arial', 10, 'bold'),
                                  bg='white', fg='#2c3e50', relief='raised', bd=2)
        left_frame.pack(side='left', fill='both', expand=True, padx=(0, 5))
        
        # Right panel - Result
        right_frame = tk.LabelFrame(main_frame, text="🎯 Kết quả phát hiện", font=('Arial', 10, 'bold'),
                                   bg='white', fg='#2c3e50', relief='raised', bd=2)
    ...
    def create_left_panel(self, parent):
        ...
        # Select button
        select_btn = tk.Button(parent, text="📂 Chọn ảnh", font=('Arial', 12, 'bold'),
                              bg='#3498db', fg='white', command=self.select_image,
                              relief='flat', padx=20, pady=10)
        select_btn.pack(pady=10)
        
        # Image display
        self.input_image_label = tk.Label(parent, text="Chưa có ảnh", 
                                         font=('Arial', 12), bg='#ecf0f1', fg='#7f8c8d',
                                         width=40, height=15)
        self.input_image_label.pack(pady=10, padx=10)
        
        # File info
        self.file_info_label = tk.Label(parent, text="", font=('Arial', 9), 
                                       bg='white', fg='#7f8c8d')
    ...
    def create_right_panel(self, parent):
        ...
        # Result image
        self.result_image_label = tk.Label(parent, text="Chờ xử lý...", 
                                          font=('Arial', 12), bg='#ecf0f1', fg='#7f8c8d',
                                          width=40, height=15)
        self.result_image_label.pack(pady=10, padx=10)
        
        # Results text
        self.results_text = tk.Text(parent, height=8, width=40, 
                                   font=('Arial', 10), bg='white', relief='flat',
                                   wrap=tk.WORD)
        self.results_text.pack(pady=10, padx=10, fill='both', expand=True)
        
        # Save button
        save_btn = tk.Button(parent, text="💾 Lưu kết quả", font=('Arial', 10),
    ...
    def create_controls(self):
        ...
        # Model status
        self.status_label = tk.Label(controls_frame, text="⏳ Đang tải model...", 
        ...
        tk.Label(conf_frame, text="Độ tin cậy:", font=('Arial', 10), 
                fg='white', bg='#34495e').pack(side='left')
        
        self.conf_var = tk.IntVar(value=50)
        self.conf_scale = tk.Scale(conf_frame, from_=10, to=90, orient='horizontal',
                                  variable=self.conf_var, length=150, bg='#34495e', 
                                  fg='white', highlightthickness=0)
        self.conf_scale.pack(side='left', padx=10)
        
        # Process button
        self.process_btn = tk.Button(controls_frame, text="🚀 Phát hiện", 
            ...
            if result.boxes is not None:
                ...
                for i, box in enumerate(result.boxes, 1):
                    ...
                    self.results_text.insert(tk.END, 
                                           f"{i}. {class_name}\n"
                                           f"   Độ tin cậy: {confidence_percent:.1f}% {status}\n\n")
            else:
