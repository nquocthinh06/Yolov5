...
# ============== PAGE CONFIG ==============
st.set_page_config(
    page_title="🚦 YOLOv5 Traffic Detection", page_icon="🚦", layout="wide", initial_sidebar_state="expanded"
)
...
with st.sidebar:
    ...
    tab_names = ["🎯 Detect & Surveillance", "🎓 Train Model", "✅ Validate System", "📦 Export Model"]

    st.session_state.tab = st.radio(
        "Select Module", range(len(tab_names)), format_func=lambda x: tab_names[x], label_visibility="collapsed"
    )
...
if st.session_state.tab == 0:
    ...
    with col_input:
        ...
        # Source selection
        input_mode = st.radio("📍 Select Source", ["📤 Upload Image", "📹 Upload Video", "🎥 Webcam"], horizontal=False)

        if input_mode != "🎥 Webcam":
            if input_mode == "📤 Upload Image":
                uploaded = st.file_uploader("Select image (JPG, PNG)", type=["jpg", "jpeg", "png"], key="img_upload")
                if uploaded:
                    st.success(f"✅ Image: **{uploaded.name}**")
            else:
                uploaded = st.file_uploader("Select video (MP4, AVI)", type=["mp4", "avi", "mov"], key="vid_upload")
                if uploaded:
        ...
        confidence = st.slider(
            "🎯 Confidence Threshold", 0.0, 1.0, 0.50, step=0.05, help="Minimum confidence to report a detection"
        )
        st.write(f"**Current:** `{confidence:.2f}`")

        iou = st.slider(
            "📏 IoU Threshold (NMS)", 0.0, 1.0, 0.45, step=0.05, help="Intersection over Union threshold for NMS"
        )
        ...
        with col_b1:
            if st.button("🚀 Run Detection", key="detect_btn", use_container_width=True):
                with st.spinner("⏳ Processing detection..."):
                    ...
                    st.session_state.detection_results = {"count": 6, "confidence": confidence, "time": "145ms"}
...
[14:33:15] Checkpoint saved: best.pt""",
                language="log",
            )
        ...
        fig.add_trace(
            go.Scatter(
                x=epochs_list, y=loss_list, mode="lines+markers", name="Loss", line=dict(color="#ef4444", width=3)
            )
        )
        fig.add_trace(
            go.Scatter(
                x=epochs_list, y=map_list, mode="lines+markers", name="mAP50", line=dict(color="#10b981", width=3)
            )
        )
