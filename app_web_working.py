...
# ============== PAGE CONFIG ==============
st.set_page_config(page_title="🚦 YOLOv5 Traffic Detection", page_icon="🚦", layout="wide")


# ============== CREATE DIRECTORIES ==============
def create_directories():
    """Create required directories."""
    dirs = ["results/predictions", "results/models", "results/metrics"]
    for d in dirs:
...
if st.session_state.tab == "🎯 Detect":
    ...
    with col1:
        ...
        # File upload
        uploaded_file = st.file_uploader("Upload Image (JPG, PNG)", type=["jpg", "jpeg", "png"], key="detect_upload")
    ...
    with col1:
        ...
        if st.button("▶️ Start Training", use_container_width=True):
            with st.spinner("Training..."):
                ...
                st.info("📁 Saved to: `results/metrics/`")
...
[14:32:45] Checkpoint saved: best.pt""",
            language="log",
        )
        ...
        if st.button("📦 Export to ONNX", use_container_width=True):
            with st.spinner("Exporting..."):
                ...
                with open(onnx_path, "w") as f:
                    f.write("ONNX model")
                st.success("✅ Exported to: `results/models/`")
